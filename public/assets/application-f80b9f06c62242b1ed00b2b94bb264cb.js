...
captureIframeLinks : function() {
    ...
    for(i = 0; i < links.length; i++) {
        ...
        links[i].onclick = function(){forceiFrame.pushURLtoParent(this.href);};
    }
}

pushURLtoParent : function(href) {
    var dest1 = parent.document.getElementById('iFrameChildURLPlaceholder');
    var dest2 = parent.document.getElementById('iFrameChildTitlePlaceholder');
    dest1.value = href;
    dest2.value = ''+document.title;
    dest1.onchange(); // calls updateParentURL
}

updateParentURL : function (childUrl) {
    var res = forceiFrame.checkIfInScope(childUrl); // returns the relative URL if in scope, returns false if out of scope
    if(res){
        // in scope
        var encurl = forceiFrame.encodeUrl(res); // compresses the URL then encodes it in Base64
    ...
    var dest2 = document.getElementById('iFrameChildTitlePlaceholder');
    document.title = dest2.value;
}
